...
export default function (pi: ExtensionAPI) {
  pi.registerCommand("summarize", {
    ...
    handler: async (_args, ctx) => {
      ...
      // Use Pi's active model and its resolved authentication. This supports
      // subscription providers such as openai-codex as well as API-key providers.
      const model = ctx.model;
      if (!model) {
        if (ctx.hasUI) ctx.ui.notify("No active model selected", "warning");
        return;
      }
      ...
      if (!auth.apiKey) {
        if (ctx.hasUI) {
          ctx.ui.notify(
            `No authentication token for ${model.provider}/${model.id}`,
            "warning",
          );
        }
        return;
